...
|------|---------|---------|
| feat | `feat/<slug>` | `feat/spring-boot-c4` |
| fix | `fix/<slug>` | `fix/spring-ai-context` |
...
```
main
 └── docs/spring-ai-c4-context        # PR #1 → base: main
      └── fix/spring-ai-context-typo  # PR #2 → base: docs/spring-ai-c4-context
```
...
| Priority | Source |
|----------|--------|
| 1 | Upstream project official docs (Spring AI, Spring Security, …) |
| 2 | C4 / PlantUML docs when the change is about modeling or rendering |
| 3 | **arXiv** abs pages when citing a method or paper |
| 4 | Research / OSS release notes or upstream GitHub implementation docs |

Avoid: random blogs, undated tweets, marketing landing pages (unless no primary source exists — then note why).

### AI / model reference set (when relevant)

For Spring AI, RAG, MCP, LLM, or algorithm-related architecture notes, prefer a fuller set when sources exist:

1. Academic / paper page (e.g. arXiv abs)
2. Official vendor docs or release notes
3. Upstream GitHub or implementation docs
...
```
docs: refine Spring AI system context C4

Clarify ChatClient and MCP boundaries so newcomers can read C1 before containers.
